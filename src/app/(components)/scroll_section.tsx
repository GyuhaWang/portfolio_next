...
export default function ScrollSection({index, avaliable}:{index: number,avaliable : boolean}){
    const {scrollToRight, scrollToLeft, scrollToUp, scrollToDown, scrollToUpLeft, scrollToUpRight, scrollToDownLeft, scrollToDownRight} = useControlledScroll();
...
{index!=4&&avaliable &&  <button onClick={()=>handleClick(index)} className='relative h-full w-full flex items-center justify-center  
 opacity-20 transition duration-1000 hover:opacity-100
        ...
        <p className='invisible group-hover:visible text-xs text-gray-300'>click</p>
        </div>
